...
/**
  ...
  */
class DecisionTreeSMACMetadata(
                                           val numFeatures: Int,
...
object DecisionTreeSMACMetadata extends Logging {
  ...
  def buildMetadata(
    ...
    if (strategy.categoricalFeaturesInfo.nonEmpty) {
      ...
      //Modified: To allow to work with few training instances and several categories (though not considered for split
      //at first) //Instead of strategy.maxBins it was maxPossibleBins
      require(maxCategoriesPerFeature <= strategy.maxBins,
        s"DecisionTree requires maxBins (= ${strategy.maxBins}) to be at least as large as the " +
