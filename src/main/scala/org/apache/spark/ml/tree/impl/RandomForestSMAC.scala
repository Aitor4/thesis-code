...
/**
  ...
  */
object RandomForestSMAC extends Logging {
  ...
  def run(
    ...
    val metadata =
      DecisionTreeSMACMetadata.buildMetadata(retaggedInput, strategy, numTrees, featureSubsetStrategy)
    instr match {
  ...
  private[tree] def selectNodesToSplit(
    ...
    while (nodeStack.nonEmpty && !groupDone) {
      ...
      val nodeMemUsage = RandomForestSMAC.aggregateSizeForNode(metadata, featureSubset) * 8L
