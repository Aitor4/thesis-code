...
/**
  ...
  */
class GaussianProcess extends SmboModel {
  ...
  val subsetCandidates=10  //: TODO using 10 for saving computation time, use 20??
  ...
  var optimIterations = 0 //TODO: Remove
  ...
  def fit(xtrain: DenseMatrix[Double], ytrain: DenseVector[Double]) : this.type = {
    ...
    if(init){ //Only setting default hyperparameters before sampling on initialization
  ...
  def predict(x: DenseVector[Double],mean:Double, amp2:Double, noise:Double,
    ...
    val cov = amp2*(1.0+1e-6) -  sum(pow(Lk,2),Axis._0) //Own kernel is 1 for the same vector, that why no computed

    (mu(0), sqrt(cov(0, 0)))     //Returns only first element (prediction) because only predicting one element at a time
...
//    println("Time calculating own kernel"+(time1-time0))
    val L = cholesky(K)
//    val time2 = System.currentTimeMillis()
//    println("Time cholesky: "+(time2-time1))
    val Xtest = x.toDenseMatrix.t

//    val time3 = System.currentTimeMillis()
    val K_s = amp2*kernel(Xtrain, Xtest, lengthScale)
//    val time4 =  System.currentTimeMillis()
//    println("Time cross kernel: "+(time4-time3))
    val Lk = L \ K_s
//    val time5 =  System.currentTimeMillis()
//    println("Time solve system Lk: "+(time5-time4))

    val mut = Lk.t * (L \ (Ytrain-mean))+mean
//    val time6 =  System.currentTimeMillis()
//    println("Time mu calculation: "+(time6-time5))
...
//    println("Time cov calculation: "+(time7-time6))
    val mu = mut(0)
...
//    println("Time calculating improvement: "+(time9-time8))
...
//      println("Time final gradient calculation: "+(time14-time13))
  ...
  val objective = new DiffFunction[DenseVector[Double]] {
    def calculate(x: DenseVector[Double]) = {
      ...
      var it = 0
//      println("Iteration of this point: "+optimIterations)
      ...
      val time1=System.currentTimeMillis()
//      println("Time object opt: "+(time1-time0))
      optimIterations += 1
  ...
  def next(originalCandidates: DenseMatrix[Double]): DenseVector[Double] = {
    ...
    def optimize (candidateN:Int) = Future{
      try{
        ...
        case e: AssertionError => { //TODO: With few data, seldom the optimization throws an exception
    ...
    for (p <- 0 until subsetCandidates){
      //      println("Point is: "+candidates(::,indxs(p)))
      //      println("EI: "+overallEi(indxs(p)))
      //      println("Optimum point is: "+result(p))
  ...
  def topNext(n: Int, candidates: DenseMatrix[Double]): Array[DenseVector[Double]] = {
    ...
    //val lie = breeze.linalg.max(Ytrain)
