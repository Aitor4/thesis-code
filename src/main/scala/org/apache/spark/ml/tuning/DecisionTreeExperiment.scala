...
import org.apache.spark.ml.tuning.TransformationUtils.integerTransformation
...
object DecisionTreeExperiment {
   ...
   def main(args: Array[String]): Unit = {
    ...
    val params = Array(rf.impurity, rf.maxDepth, rf.minInfoGain, rf.minInstancesPerNode).map(_.asInstanceOf[Param[Any]]) //Array(svm.regParam, svm.fitIntercept, svm.standardization).map(_.asInstanceOf[Param[Any]])

    val categories = Array(2, -13, 0,0)
    ...
    def modelTransformations(input: DenseVector[Double]): Array[Any] = {
      ...
      result(0) = if(input(0)<0.5) "gini" else "entropy"
      result(1) = integerTransformation(input(1),13,3)
      result(2) = input(2)
      result(3) = integerTransformation(input(3),50,1)
      return result
    }
...
/*    if (args(1) == "0" || args(1) == "-1") {
     ...
     for (t <- 0 until 5){
      if (args(1) == "0" || args(1) == "1") {
        ...
        val loopR0 = new SmboLoop(random0, rf, evaluator, training, validation, params)
    ...
    if (args(1) == "0" || args(1) == "2") {
      ...
      val loops = new SmboLoop(gps, rf, evaluator, training, validation, params)
    ...
    if (args(1) == "0" || args(1) == "3") {
      ...
      val loop20 = new SmboLoop(smac0, rf, evaluator, training, validation, params)
    ...
    if (args(1) == "0" || args(1) == "4") {
      ...
      val loopR = new SmboLoop(random, rf, evaluator, training, validation, params)
    ...
    if (args(1) == "0" || args(1) == "5" || args(1)=="8") {
      println("STARTING SMAC SH ----------")
      val smac = new RandomForestSurrogate(sparkSession.sqlContext, categories)
      val loop2 = new SmboLoop(smac, rf, evaluator, training, validation, params)
    ...
    if (args(1) == "0" || args(1) == "6" || args(1)=="8") {
      println("STARTING SPEARMINT SH----------")
      val gp = new GaussianProcess()
      val loop = new SmboLoop(gp, rf, evaluator, training, validation, params)
